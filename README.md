...
[![tests](https://github.com/nathanaelhub/backprop-from-scratch/actions/workflows/ci.yml/badge.svg)](https://github.com/nathanaelhub/backprop-from-scratch/actions/workflows/ci.yml)
...
pip install -r requirements-dev.txt && pytest    # run the test suite
```
...
`pytest` covers the parts that have to be right for the rest to mean anything:

- **the gradient check** — analytic gradients match finite differences to < 1e-4 on both the 2-class and 3-class networks (the proof that `backward()` is correct)
- **primitives** — `matmul` against known products, `transpose`, `col_sums`, ReLU and its mask
- **softmax** — rows sum to 1, and the max-subtraction trick survives huge logits without overflow
- **cross-entropy** — ~0 for a confident correct call, `log k` for a uniform guess
- **training** — both tasks reach ≥ 95% accuracy, and a single step provably lowers the loss
- **reproducibility** — a fixed seed gives identical initial weights

```bash
pip install -r requirements-dev.txt
pytest -q          # 15 tests, ~1s
```
...
├── tests/          # pytest suite (gradient check, primitives, convergence)
├── assets/         # generated images
└── .github/        # CI: runs the training smoke test + pytest on every push
```
